...
function solution(video_len, pos, op_start, op_end, commands) {
  ...
  const posDateToString = `${String(posDate.getMinutes()).padStart(
    2,
    "0"
  )}:${String(posDate.getSeconds()).padStart(2, "0")}`;
...
function doCommand(command, posDate, videoDate, opStartDate, opEndDate) {
  ...
  const totalVideoSeconds =
    videoDate.getMinutes() * 60 + videoDate.getSeconds();
  const totalOpEndSeconds =
    opEndDate.getMinutes() * 60 + opEndDate.getSeconds();
  const totalOpStartSeconds =
    opStartDate.getMinutes() * 60 + opStartDate.getSeconds();
  ...
  if (
    updatedPosSeconds >= totalOpStartSeconds &&
    updatedPosSeconds <= totalOpEndSeconds
  ) {
    posDate = new Date(opEndDate); // 오프닝 끝으로 이동
  }
...
function solution(video_len, pos, op_start, op_end, commands) {
  ...
  let opStartDate = changeStringToDate(op_start);
  let opEndDate = changeStringToDate(op_end);
  let zeroDate = changeStringToDate("00:00");
...
function changeStringToDate(dateString) {
  ...
  date.setMinutes(parseInt(dateArray[0]));
  date.setSeconds(parseInt(dateArray[1]));
...
function doCommand(
  ...
  if (posDate >= opStartDate && posDate <= opEndDate) {
    posDate = new Date(opEndDate);
  }

  if (command === "next") {
    posDate.setSeconds(posDate.getSeconds() + 10);

    if (videoDate <= posDate) {
      posDate = new Date(videoDate);
    }
  } else if (command === "prev") {
    posDate.setSeconds(posDate.getSeconds() - 10);

    if (posDate <= zeroDate) {
      posDate = new Date(zeroDate);
    }
  }
