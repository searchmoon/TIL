...
function solution(a, b, c, d) {
  let sortArr = [a, b, c, d].sort((a, b) => a - b);
  let p;
  let q;
  let r;
  // 4개의 수가 같을때
  if (sortArr[0] === sortArr[3]) {
    p = sortArr[0];
    return 1111 * p;
  }

  // 3개의 수가 같을 때
  if (sortArr[0] === sortArr[2] || sortArr[1] === sortArr[3]) {
    if (sortArr[2] !== sortArr[3]) {
      p = sortArr[0];
      q = sortArr[3];
    } else {
      p = sortArr[3];
  ...
  //2개씩 수가 같을 떄
  if (sortArr[0] === sortArr[1] && sortArr[2] === sortArr[3]) {
    p = sortArr[0];
    q = sortArr[2];
  ...
  if (sortArr[0] === sortArr[1]) {
    ...
    r = sortArr[3];
    return q * r;
  } else if (sortArr[1] === sortArr[2]) {
    q = sortArr[0];
    r = sortArr[3];
    return q * r;
  } else if (sortArr[2] === sortArr[3]) {
    q = sortArr[2];
    r = sortArr[1];
    return q * r;
  }
...
function solution(k, score) {
  ...
  for (let scoreNum of score) {
    tempArr.push(scoreNum);
    ...
    let minNum = tempArr[tempArr.length - 1];
    answer.push(minNum);
  }
